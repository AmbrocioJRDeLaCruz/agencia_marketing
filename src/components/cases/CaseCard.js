...
function CaseCard({data, index}){
    return(
        ...
        onMouseEnter={()=>{
            ...
            title_element.classList.add('text-blue-900');
            const img = document.getElementById(data.id)
            img.classList.add('object-scale-down')
        }}
        onMouseLeave={()=>{
            const title_element = document.getElementById(index);
            title_element.classList.remove('text-blue-900');
            const img = document.getElementById(data.id)
        ...
        className="flex flex-col  overflow-hidden rounded-lg shadow-lg">
                ...
                <div className="flex flex-1 flex-col justify-between bg-white p-6">
                  <div className="flex-1">
                    <p className="text-kl font-medium text-gray-500">
                    ...
                    <a href={data.href} className="mt-2 block">
                      ...
                      <p className="mt-3 text-2xl leading-19 text-gray-500">{data.description}</p>
                    </a>
                  </div>
                </div>
              </Link>
